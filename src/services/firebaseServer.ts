...
const getPlaylistById = (id: number) => {
	const response = api
		.get(`playlists/${id}.json?_sort=playedTimes&_order=desc`)
		.then(response => response.data)
...
const getTop4Playlists = () => {
	const response = api
		.get('playlists.json?_sort=playedTimes&_order=desc&_limit=4')
		.then(response => response.data)
...
const getMusicsByPlaylistId = async (playlistId: number) => {
	const response = await api
		.get(`musics.json?playlistId=${playlistId}`)
		.then(response => response.data)
...
const getLikedPlaylists = async (uid: string) => {
	...
	const response = await api
		.get(`likedPlaylists.json?uid=${uid}`)
		.then(response => response.data)
...
const getPlaylistWasLiked = async (playlistId: number, uid: string) => {
	const response = await api
		.get(`likedPlaylists.json?uid=${uid}&playlistId=${playlistId}`)
		.then(response => {
...
const updatePlaylistPlayedTimes = async (station: PlaylistType) => {
	station.playedTimes++
	const response = await api
		.put(`playlists/${station.id}.json`, station)
		.then(response => response.data)
